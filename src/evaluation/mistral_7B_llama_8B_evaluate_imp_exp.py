...
def evaluate(model_dir, data_dir, split, pred_dir, model_name, max_length=2048, max_new_tokens=2048):
    ...
    def count_valid_spans(data):
        # Case-insensitive patterns for implicit and explicit tags
        implicit_pattern = re.compile(r"<implicit>.*?</implicit>", re.IGNORECASE)
        explicit_pattern = re.compile(r"<explicit>.*?</explicit>", re.IGNORECASE)
    ...
    def extract_spans(data):
        spans = []
        # Case-insensitive patterns with DOTALL 
        implicit_pattern = re.compile(r"<implicit>\s*(.*?)\s*</implicit>", re.DOTALL | re.IGNORECASE)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate fine-tuned Mistral or LLaMA on implicit/explicit tagging")
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = f"results_cmv_{args.model_name}_finetune_imp_exp"
